...
async fn main(spawner: Spawner) {
    ...
    // Setup HTTPS client
    let client = simplyplural::init_http_client(wifi_stack, peripherals.RSA);

    spawner.must_spawn(main_loop(epd, &mut *display, spi_bus, client, font, delay));
}

#[embassy_executor::task]
async fn main_loop(
    mut epd: EpdDisplay,
    display: &'static mut EpdBuffer,
    mut spi_bus: SpiBus,
    mut http: simplyplural::HttpClient,
    font: Font<'static>,
    mut delay: Delay,
) {
    ...
    loop {
        let text = match simplyplural::fetch_current_front_name(&mut http).await {
            Ok(text) => text,
            Err(err) => {
                log::info!("{err:?}");
